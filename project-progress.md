...
- Current phase: third-source rollout and catalog confidence expansion
...
| Ingestion and canonical catalog pipeline | In progress | 84% | Multi-source ingestion now relies on live `Netflix-Codes`, `Teen Vogue`, and the `GitHub Netflix-Codes README` adapter for public catalog truth, preserves provenance, and keeps the starter dataset out of the public category contract. |
...
- Canonical data source: In progress - the catalog now uses live `Netflix-Codes`, `Teen Vogue`, and `GitHub Netflix-Codes README` sources for public category truth, while the curated starter dataset stays recommendation-only.
...
- The first real ingestion slice now imports starter recommendations plus live `Netflix-Codes`, `Teen Vogue`, and `GitHub Netflix-Codes README` category sources into MongoDB, stores raw provenance records, merges duplicate codes by source priority, and switches public discovery reads to canonical collections.
...
- Current local source-expansion work adds a GitHub-hosted markdown catalog as the third approved ingestion source.

### Not Done Yet

- Define the exact approved public sources and review criteria for ingestion.
- Define the fourth approved remote source after `GitHub Netflix-Codes README`.
...
- Data readiness is stronger with a third source, but catalog completeness still depends on adding more approved sources beyond the current three-source set.
...
- Finalize the approved-source checklist now that the third source is in code.
