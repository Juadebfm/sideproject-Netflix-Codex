...
- Current phase: second remote-source rollout and live catalog expansion
...
| Backend public API and data contract | In progress | 84% | Search, recommendations, category detail, analytics intake, health, and cron kickoff now read from canonical Mongo-backed collections with explicit unavailable errors and CORS support. |
| Ingestion and canonical catalog pipeline | In progress | 78% | Multi-source ingestion now merges the starter source with live `Netflix-Codes` and `Teen Vogue` adapters, dedupes by `netflixCode`, preserves provenance, and seeds a much larger catalog into MongoDB. |
...
- Search and category discovery: In progress - public search and category endpoints now read from canonical Mongo-backed data after ingestion.
...
- Canonical data source: In progress - the catalog now merges the curated starter dataset with live `Netflix-Codes` and `Teen Vogue` sources into raw-source and canonical Mongo collections.
...
- The first real ingestion slice now imports the curated starter dataset plus live `Netflix-Codes` and `Teen Vogue` sources into MongoDB, stores raw provenance records, merges duplicate codes by source priority, and switches public discovery reads to canonical collections.
